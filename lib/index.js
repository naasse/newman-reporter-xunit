	...
	newman.on('beforeDone', function () {
		...
		_.forEach(executions, function (execution) {
				...
				_.forEach(propertyValues, function (propertyItem) {
					...
					if (propertyItem.key.toLowerCase().startsWith(REQ_NAME_KEY)) {
						if (propertyItem.value != null
							...
							&& propertyItem.key.toLowerCase() === REQ_NAME_KEY + execution.cursor.iteration + execution.item.name) {
							requestName = propertyItem.value;
						}
						console.log(propertyItem.value)
						return
					}
			...
			if (execution.cursor.cycles > 1) {
					if (packageName) {
							testsuite.att('package', 'Iteration ' + execution.cursor.iteration.toString() + SEPARATOR + packageName);
					} else {
							testsuite.att('package', 'Iteration ' + execution.cursor.iteration.toString());
					}
			} else {
